$(function () {
    ...
    async function updateMontoPorCobrar(deudores, capas, tipos_anomalia, anomalias, clientes) {
        const signal = controller.signal;
        try {
            let data = `dashboard/getMontoPorCobrar/${fecha_inicia}/${fecha_final}/${(deudores=="")?null:deudores}/${(capas=="")?null:capas}/${(tipos_anomalia=="")?null:tipos_anomalia}/${(anomalias=="")?null:anomalias}/${(clientes=="")?null:clientes}`
    ...
    async function updateFacturasPorCobrar(deudores, capas, tipos_anomalia, anomalias, clientes) {
        const signal = controller.signal;
        try {
            let data = `dashboard/getFacturasPorCobrar/${fecha_inicia}/${fecha_final}/${(deudores=="")?null:deudores}/${(capas=="")?null:capas}/${(tipos_anomalia=="")?null:tipos_anomalia}/${(anomalias=="")?null:anomalias}/${(clientes=="")?null:clientes}`
    ...
    async function updateTotalDeudores(deudores, capas, tipos_anomalia, anomalias, clientes) {
        const signal = controller.signal;
        try {
            let data = `dashboard/getTotalDeudores/${fecha_inicia}/${fecha_final}/${(deudores=="")?null:deudores}/${(capas=="")?null:capas}/${(tipos_anomalia=="")?null:tipos_anomalia}/${(anomalias=="")?null:anomalias}/${(clientes=="")?null:clientes}`
    ...
    async function renderGraficaSaldoPorDivision(deudores, capas, tipos_anomalia, anomalias, clientes) {
        const signal = controller.signal;
        try {
            let data_ = `dashboard/getEstatusVencimiento/${fecha_inicia}/${fecha_final}/${(deudores=="")?null:deudores}/${(capas=="")?null:capas}/${(tipos_anomalia=="")?null:tipos_anomalia}/${(anomalias=="")?null:anomalias}/${(clientes=="")?null:clientes}`
            // Mostramos el loader
    ...
    // Saldo por estatus
    async function renderGraficaSaldoPorEstatus(deudores, capas, tipos_anomalia, anomalias, clientes) {
        const signal = controller.signal;
        try {
            let data_ = `dashboard/getDataGraficaSaldoPorEstatus/${fecha_inicia}/${fecha_final}/${(deudores=="")?null:deudores}/${(capas=="")?null:capas}/${(tipos_anomalia=="")?null:tipos_anomalia}/${(anomalias=="")?null:anomalias}/${(clientes=="")?null:clientes}`
            // Mostramos el loader
    ...
    async function renderGraficaSaldoPorCapaAging(deudores, capas, tipos_anomalia, anomalias, clientes) {
        const signal = controller.signal;
        try {
            let data_ = `dashboard/getSaldoCapaAging/${fecha_inicia}/${fecha_final}/${(deudores=="")?null:deudores}/${(capas=="")?null:capas}/${(tipos_anomalia=="")?null:tipos_anomalia}/${(anomalias=="")?null:anomalias}/${(clientes=="")?null:clientes}`
            // Mostramos el loader
    ...
    // Saldo por Región
    async function renderGraficaSaldoPorRegion(deudores, capas, tipos_anomalia, anomalias, clientes) {
        const signal = controller.signal;
        try {
            let data_ = `dashboard/getSaldoPorRegion/${fecha_inicia}/${fecha_final}/${(deudores=="")?null:deudores}/${(capas=="")?null:capas}/${(tipos_anomalia=="")?null:tipos_anomalia}/${(anomalias=="")?null:anomalias}/${(clientes=="")?null:clientes}`
            // Mostramos el loader
    ...
    async function renderTablaTop20DeudoresConSaldoVencido(deudores, capas, tipos_anomalia, anomalias, clientes) {
        const signal = controller.signal;
        try {
        ...
        `);
            let data_ = `dashboard/getDataTablaTop20DeudoresSaldoVencido/${fecha_inicia}/${fecha_final}/${(deudores=="")?null:deudores}/${(capas=="")?null:capas}/${(tipos_anomalia=="")?null:tipos_anomalia}/${(anomalias=="")?null:anomalias}/${(clientes=="")?null:clientes}`
            let peticion = await fetch(url + data_, { signal });
            let response = await peticion.json();
            $("#bodyTopDeudores").html("");
            let i = 1;
    ...
    // KPI Current %
    async function renderKPI(deudores, capas, tipos_anomalia, anomalias, clientes) {
        const signal = controller.signal;
        try {
            // Mostramos el loader
            ...
            let data_ = `dashboard/getKPI/${fecha_inicia}/${fecha_final}/${(deudores=="")?null:deudores}/${(capas=="")?null:capas}/${(tipos_anomalia=="")?null:tipos_anomalia}/${(anomalias=="")?null:anomalias}/${(clientes=="")?null:clientes}`
            let peticion = await fetch(url + data_, { signal });
            let response = await peticion.json();
            // Ocultamos el loader
    ...
    // DSO %
    async function renderDSO(deudores, capas, tipos_anomalia, anomalias, clientes) {
        const signal = controller.signal;
        try {
            // Mostramos el loader
            ...
            let data_ = `dashboard/getDSO/${fecha_inicia}/${fecha_final}/${(deudores=="")?null:deudores}/${(capas=="")?null:capas}/${(tipos_anomalia=="")?null:tipos_anomalia}/${(anomalias=="")?null:anomalias}/${(clientes=="")?null:clientes}`
            let peticion = await fetch(url + data_, { signal });
            let response = await peticion.json();
            // Ocultamos el loader
    ...
    async function cargarCapaAgin() {
        ...
        try {
            ...
            let peticion = await fetch(url + "dashboard/getCapaAgin", { signal });
            let response = await peticion.json();
            for (let item of response) {
                let option = document.createElement("option")
    ...
    async function cargarTipoAnomalias() {
        ...
        try {
            ...
            let peticion = await fetch(url + "dashboard/getTiposAnomalia", { signal });
            let response = await peticion.json();
            for (let item of response) {
                let option = document.createElement("option")
    ...
    async function cargarAnomalias() {
        ...
        try {
            ...
            let peticion = await fetch(url + "dashboard/getAnomalias", { signal });
            let response = await peticion.json();
            for (let item of response) {
                let option = document.createElement("option")
    ...
    function exportarDatos() {
        ...
        let peticion = url + `dashboard/getexportar/${fecha_inicia}/${fecha_final}/${(deudores=="")?null:deudores}/${(capas=="")?null:capas}/${(tipos_anomalia=="")?null:tipos_anomalia}/${(anomalias=="")?null:anomalias}/${(clientes=="")?null:clientes}`
    ...
    function aplicarFiltros() {
        ...
        updateMontoPorCobrar(deudores, capas, tipos_anomalia, anomalias, clientes);
        updateFacturasPorCobrar(deudores, capas, tipos_anomalia, anomalias, clientes);
        updateTotalDeudores(deudores, capas, tipos_anomalia, anomalias, clientes);

        renderGraficaSaldoPorDivision(deudores, capas, tipos_anomalia, anomalias, clientes);
        renderGraficaSaldoPorEstatus(deudores, capas, tipos_anomalia, anomalias, clientes)
        renderGraficaSaldoPorCapaAging(deudores, capas, tipos_anomalia, anomalias, clientes)
        renderGraficaSaldoPorRegion(deudores, capas, tipos_anomalia, anomalias, clientes)

        renderTablaTop20DeudoresConSaldoVencido(deudores, capas, tipos_anomalia, anomalias, clientes)
        renderDSO(deudores, capas, tipos_anomalia, anomalias, clientes)
        renderKPI(deudores, capas, tipos_anomalia, anomalias, clientes)
    }
